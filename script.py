                                        ...
                                        for para in req[header][itm]:
                                            if para == "mobileNumber":
                                                ...
                                                if p.match(str(req[header][itm][para])):
                                                    ...
                                                    respo = {"fulfillmentText": "Please enter a valid 10 digit mobile number","fulfillmentMessages": [],"source": "example.com","payload": {},"outputContexts": [ ],"followupEventInput": {}}
                                                    return jsonify(respo)
                                    ...
                                    if itm == "parameters":
                                        for para in req[header][itm]:
                                            if para == "otp":
                                                ...
                                                print (req[header][itm][para])
                                                if p.match(str(req[header][itm][para])):
                                                    respo = {"fulfillmentText": "Otp Verified successfully..... what are you looking for ? Apply Loan,Track Your Loan Status,Customer Service,Track Your SR Status","fulfillmentMessages": [{"card": {"title": "Otp Verified successfully","subtitle": "what are you looking for ?","buttons": [{"text": "Apply Loan","postback": "apply loan"},{"text": "Track Loan status","postback": "Track Loan status"}]}},{"card": {"buttons": [{"text": "Customer Service","postback": "Customer Service"},{"text": "Track SR status","postback": "Track SR status"}]}}],"source": "example.com","payload": {},"outputContexts": [ ],"followupEventInput": {}}
                                                    return jsonify(respo)                    
                                                else:
